...
| 16 | `code/bank_000.s` | `IF LAB` routes the `$06`, `$07` and `$08` jump-table entries through `LabStateHook` | 3 (hook) | `$06` and `$07` are the Lab title screen — the original's screen with the Lab's own init and 1 PLAYER / 2 PLAYER handling. They have to be **these** states: `SerialFunc0_titleScreen` (`$0078`) assigns a multiplayer role only while `hGameState` is `$07`, and forces every other state back to `$06`, so this is where the passive ping has to come from. `$08` paints the Lab menu, and is also where B on a level select comes back to. The init's clears, walls and floor are transcribed, because the falling piece collides against the buffer they are written into. See `docs/decisions/0007`. | 6. Zero when `LAB=0`. |
...
| 22 | `code/bank_000.s` | `IF LAB` routes the `$0E` jump-table entry (game type screen main) through `LabStateHook` | 3 (hook) | The Lab menu runs here, paired with `$08` the way the original paired them. The menu moved off the title screen so the title screen could come back: 2 PLAYER has to be chosen in `$07`, the only state the original's serial code will assign a role in. | 2. Zero when `LAB=0`. |
| 23 | `code/bank_000.s` | `IF LAB` points the VBlank handler's `ldh [rSCY], a` at `hUnusedFFA4` instead | 3 (hook) | VBlank zeroes `rSCY` at the end of every frame, so nothing can scroll the background — the Lab menu writes the register and the next VBlank throws it away. The menu is a list taller than the screen, so it has to scroll. One byte, the operand alone: `A` is still zero there, so the `inc a` after it still stores exactly 1 into `hVBlankInterruptFinished` and the main loop's wait is unchanged. Nothing else depends on the write — the only other places `rSCY` is touched are `rSCX` beside it and `Reset` (`$0223`), and no original screen ever sets it non-zero. The Lab clears it on the way out of the menu. | 1. Zero when `LAB=0`. |
| 24 | `code/bank_000.s` | `IF LAB` routes the `$12` jump-table entry (B-type level select) through `LabStateHook` | 2 (hook) | `GameState08_GameMusicTypeInit` (`$1452`) was the only place in the ROM that loaded the shared tileset, and every screen after it inherited. The Lab menu replaced that screen and loads Tolstoj's artwork instead, so each screen downstream fetches its own — inside the LCD-off window its init already opens, which is what keeps a swapped tileset from ever being displayed. The Lab draws nothing on this screen; the hook exists only for the load. | 2. Zero when `LAB=0`. |
